...
let getElementEvents runState =
    function
    | Button config -> 
        let rect = rectFor (Button config)
        if not <| contains runState.mouse.position (pointsFor rect)
        then None
        else
...
let private getButtonView runState config =
    ...
    [
        ...
        yield Text (config.text.font, config.text.lines.[0], centre, config.text.size, Centre, colours.text)
    ]
...
//         let padding = height / (2 + config.text.lines.Length + 1)

//         let origin = if config.text.lines.Length = 1 then Left else TopLeft
//         yield Paragraph (config.text.font, config.text.lines, (x + padding, y + padding), config.text.size, origin, colours.text)
//     ]
